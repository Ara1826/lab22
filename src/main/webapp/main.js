...
function drawGraph(R) {
    ...
    // Triangle (lower left)
    ctx.fillStyle = "rgba(255,247,91,0.32)"; // blue with 10% opacity
    ctx.beginPath();
    ctx.moveTo(width / 2, height / 2);
    ctx.lineTo(width / 2 +  dynamicScalingFactor * (R/2), height / 2); // Правая точка
    ctx.lineTo(width / 2, height / 2 + R * dynamicScalingFactor); // Верхняя точка
    ctx.closePath();
    ctx.fill();
    ctx.strokeStyle = "#dac62b";
    ctx.stroke();

    // Rectangle (lower right)
    ctx.fillStyle = "rgba(132,221,211,0.2)"; // yellow with 10% opacity
    ctx.fillRect(width / 2 - R / 2 * dynamicScalingFactor, height / 2, R / 2 * dynamicScalingFactor, R * dynamicScalingFactor);
    ctx.strokeStyle = "#4a9587";
    ctx.strokeRect(width / 2 - R / 2 * dynamicScalingFactor, height / 2, R / 2 * dynamicScalingFactor, R * dynamicScalingFactor);
    // Semi-circle (upper left)
    ctx.fillStyle = "rgba(45,255,0,0.06)"; // green with 10% opacity
    ctx.beginPath();
...
function updateTableAndGraph(data) {
    ...
    let ParsedT = parseFloat(data.executionTime);

    // Draw the new point directly
    drawPoint(parsedX, parsedY, data.isHit);

    // Push it to clickedPoints
    clickedPoints.push({x: parsedX, y: parsedY, r: parsedR, isHit: data.isHit, executionTime: data.executionTime});
    ...
    cell5.innerHTML = data.executionTime;
}
// Ищем все кнопки с выбором X и добавляем обработчик
...
mainForm.addEventListener('click', function (e) {
    ...
    // Ищем активную кнопку X
    const xElement = document.querySelector('#x .btn.active');
    const yElement = document.querySelector('#y');
    const rElement = document.querySelector('#r');

    if (xElement && yElement && rElement) {
        const xVal = parseFloat(xElement.getAttribute('data-value'));
        ...
        if (isPointInsideArea(xVal, yVal, rVal)) {
            ...
            fetch("controller", {
                ...
                .then(function (serverAnswer) {
                    // Перенаправление на новую страницу с параметрами результата
                    const urlParams = new URLSearchParams({
                        x: serverAnswer.x,
                        y: serverAnswer.y,
                        r: serverAnswer.r,
                        isHit: serverAnswer.isHit,
                        executionTime: serverAnswer.executionTime
                    }).toString();

                    window.location.href = `result.jsp?${urlParams}`;
